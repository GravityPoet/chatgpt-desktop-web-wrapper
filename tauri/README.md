...
核心价值：当原生 ChatGPT 桌面 App 没有暴露网页版的更高思考强度选择时，这个 wrapper 让你在桌面窗口里继续使用 ChatGPT Web 的模型选择器和进阶 / high thinking 控制。这样处理复杂代码、分析、写作、研究任务时，可以主动选择更深推理，而不是停留在更快但更轻的默认模式。

它不绕过 ChatGPT 订阅、额度或账号权限，只是让你在桌面 App 里直接使用网页版能力，同时提供独立 WebView 登录态、下载、缩放、单实例等桌面体验。
